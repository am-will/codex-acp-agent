...
export async function loadPromptCommandDefinitions(
  ...
  const definitions: PromptCommandDefinition[] = [];

  for (const entry of entries) {
    if (!entry.isFile()) continue;
    if (!isSupportedPromptFile(entry.name)) continue;
...
export function promptArgumentNames(content: string): string[] {
  const names: string[] = [];
...
export function expandNumericPlaceholders(content: string, args: string[]): string {
  let result = "";
  let i = 0;
...
function isSupportedPromptFile(fileName: string): boolean {
  const extension = extname(fileName).toLowerCase();
  return extension === ".md" || extension === ".txt" || extension === ".prompt";
}

function parsePromptFile(fileName: string, rawContent: string): PromptCommandDefinition | undefined {
...
function parseCommandName(fileName: string, body: string): string {
